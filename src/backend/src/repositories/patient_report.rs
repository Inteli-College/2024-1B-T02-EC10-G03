...
impl PatientReportRepository {
    ...
    pub async fn create(
        ...
        self.db_client
            ...
            ._transaction()
            .run(|tx| async move {
                let new_patient_report = tx.patient_report()
                    .create(
                        patient,
                        transaction,
                        status,
                    ...
                    .exec()
                    .await?;
                Ok(new_patient_report)
            })
            .await
    }

    pub async fn update_status(&self, cuid: String, new_status: ReportStatus) -> Result<Option<PatientReport>, QueryError> {
        let result = self.db_client.get_db().patient_report().update(
            patient_report::cuid::equals(cuid.to_string()),
            vec![patient_report::status::set(new_status)],
        ).exec().await?;
